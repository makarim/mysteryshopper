...
class ShopperModel extends Model {
	...
	function getItems($con,$pageCount){
		...
		$select->from ( "sp_shopper ","*");
		
		//
		if(isset($con['order'])) $select->order ( $con['order']." desc" );
		if(isset($con['shopper']) && !empty($con['shopper'])) $select->where ( " shopper = '".$con['shopper']."'" );
		if(isset($con['s_id']) && !empty($con['s_id'])) $select->where ( " s_id = '".$con['s_id']."'" );
		if(isset($con['shopper_nickname']) && !empty($con['shopper_nickname'])) $select->where ( " shopper_nickname like '".$con['shopper_nickname']."%'" );
		if(isset($con['shopper_reg_time']) && !empty($con['shopper_reg_time'])) $select->where ( " shopper_reg_time >= '".strtotime($con['shopper_reg_time'])."'" );
		if(isset($con['shopper_reg_time1'])&& !empty($con['shopper_reg_time1'])) $select->where ( " shopper_reg_time < '".strtotime($con['shopper_reg_time1'])."'" );
	...
	function getOnlineShoppers($con,$pageCount){
		$select =$this->db->select();
		$select->from ( " onlineshopper ","*");
