		...
		if ($corp_info) {
			...
			if ($corp_info['c_password'] == md5(($passwd))) {
				if ($corp_info ['c_state'] == 1) {
					...
					if($c_name == 'test'){
					$msg = array('s'=> 200,'m'=>"ok",'d'=>$GLOBALS ['gSiteInfo'] ['www_site_url']."/client.php/hometotal");
					exit(json_output($msg));
					}else if($c_name == 'JiaShanghai'){
					$msg = array('s'=> 200,'m'=>"ok",'d'=>$GLOBALS ['gSiteInfo'] ['www_site_url']."/client.php/homehotel");
					exit(json_output($msg));
					}else{
